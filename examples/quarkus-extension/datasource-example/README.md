# EximeeBPMS Runtime Quarkus Datasource Example

This example uses Quarkus, the Supersonic Subatomic Java Framework, in combination 
with the extension  `eximeebpms-bpm-quarkus-engine`.

The Datasource example demonstrates how you can use the EximeeBPMS Platform Runtime Engine 
...
* Configure a custom datasource used by the engine.
  * The example uses an H2 in-memory database by default, but other 
    EximeeBPMS-supported datasouce configurations are available.
* Deploy a simple [approval process](src/main/resources/bpmn/process.bpmn).
* Manage transactions with Quarkus and the EximeeBPMS Platform Runtime Engine.
...
Alternatively, you can execute the [unit test case](src/test/java/org/eximeebpms/bpm/quarkus/example/datasource/ApprovalProcessTest.java).
...
The application is now runnable using `java -jar target/eximeebpms-bpm-quarkus-example-datasource-1.0.0-SNAPSHOT-runner.jar`.
