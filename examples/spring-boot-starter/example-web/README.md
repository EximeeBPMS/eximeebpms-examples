# Spring Boot Web application with embedded EximeeBPMS engine

This example demonstrates how you can build a Spring Boot Web application with the following configured:
* Embedded EximeeBPMS engine accessible via REST
...
1. To embed the EximeeBPMS engine accessible via REST API you must add following dependency to your `pom.xml`:
...
2. With Spring Boot you usually create an "application" class annotated with `@SpringBootApplication`. In order to have the EximeeBPMS process application
...
>Note: Basic authentication is configured on Spring Boot level, this is NOT an EximeeBPMS engine authentication. Check [this docs](https://docs.eximeebpms.org/manual/latest/reference/rest/overview/authentication/) 
to configure Basic Authentication for EximeeBPMS engine REST API.
