...
This is a simple plugin that showcases the plugin system of Cockpit, the process monitoring tool of [EximeeBPMS](https://docs.eximeebpms.org).
...
> Note: If you need please take a look at the [Cockpit Plug-ins](https://docs.eximeebpms.org/manual/latest/webapps/cockpit/extend/plugins/) for the basics first.

# Table of contents

1. [Integrate into EximeeBPMS Webapp](#integrate-into-eximeebpms-webapp)
...
2. There are two ways to add your plugin to the EximeeBPMS webapp.
   
   1. You can copy `./target/cockpit-sample-plugin-1.0-SNAPSHOT.jar` to the `WEB-INF/lib` folder of the EximeeBPMS webapp.
   2. You can set up a maven war overlay for the EximeeBPMS webapp.
   
   The first solution is the simplest: if you downloaded the tomcat distribution, you can copy the plugin
   jar file to the `/server/apache-tomcat-${tomcat-version}/webapps/eximeebpms/WEB-INF/lib/` folder and
   restart the server.
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <groupId>org.eximeebpms.bpm.cockpit.plugin</groupId>
...
The main entry point for a plug-in is the service provider interface (SPI) `org.eximeebpms.bpm.cockpit.plugin.spi.CockpitPlugin`. Each plug-in must provide an implementation of this class and register it via `META-INF/services`.

We will go ahead and create an implementation of that API called `SampleCockpitPlugin`.

```java
package org.eximeebpms.bpm.cockpit.plugin.sample;
...
By inheriting from `org.eximeebpms.bpm.cockpit.plugin.spi.impl.AbstractCockpitPlugin`, we ensure that the plug-in is initialized with reasonable defaults.

To register the plug-in with Cockpit, we must put its class name into a file called `org.eximeebpms.bpm.cockpit.plugin.spi.CockpitPlugin` that resides in the directory `META-INF/services`. That will publish the plug-in via the Java [ServiceLoader facilities](http://docs.oracle.com/javase/6/docs/api/java/util/ServiceLoader.html).
...
The next step consists of wiring the EximeeBPMS webapp and the process engine. To do this, we need to create a Service Provider that implements the interface `ProcessEngineProvider` and declare it in a file called `org.eximeebpms.bpm.engine.rest.spi.ProcessEngineProvider` that resides in the directory `src/test/resources/META-INF/services/`. The file should contain the following content:

```java
org.eximeebpms.bpm.cockpit.plugin.test.application.TestProcessEngineProvider
```

The `TestProcessEngineProvider` is provided with the EximeeBPMS webapp core, uses the methods of the class `org.eximeebpms.bpm.BpmPlatform` and exposes the default process engine.

The class `org.eximeebpms.bpm.cockpit.plugin.test.AbstractCockpitPluginTest` can work as a basis for Cockpit plugin tests. It initializes the Cockpit environment around each test and bootstraps a single process engine that is made available to Cockpit and the plug-in.
...
Before we can actually run the test, we need to create a `eximeebpms.cfg.xml` to be present on the class path (usually under `src/test/resources`). That file configures the process engine to be bootstrapped.
...
To implement a custom query, we will create a file `sample.xml` in the directory `org/eximeebpms/bpm/cockpit/plugin/sample/queries` with the following content:
...
<mapper namespace="cockpit.sample">
  ...
  <resultMap id="processInstanceCountMap" type="org.eximeebpms.bpm.cockpit.plugin.sample.db.ProcessInstanceCountDto">
...
```
  ...
  both the usage of a custom namespace (`cockpit.sample`) as well as the result mapping to the plug-in provided class `org.eximeebpms.bpm.cockpit.plugin.sample.db.ProcessInstanceCountDto`.

We need to define the class to which the result is mapped:

```java
package org.eximeebpms.bpm.cockpit.plugin.sample.db;
...
public class SamplePlugin extends AbstractCockpitPlugin {
  ...
  public List<String> getMappingFiles() {
    return Arrays.asList("org/eximeebpms/bpm/cockpit/plugin/sample/queries/sample.xml");
  }
}
```
...
Note that `#getQueryService()` is merely a shortcut to the service that may also be accessed via Cockpit's main entry point, the `org.eximeebpms.bpm.cockpit.Cockpit` class.
...
A server-side plug-in API consists of a root resource and a number of sub resources that are provided by the root resource. A root resource may inherit from `org.eximeebpms.bpm.cockpit.plugin.resource.AbstractPluginRootResource` to receive some basic traits. It must publish itself on the path `plugin/$pluginName` via a `@Path` annotation.

A root resource for our plug-in may look as follows:

```java
package org.eximeebpms.bpm.cockpit.plugin.sample.resources;

import jakarta.ws.rs.Path;
import jakarta.ws.rs.PathParam;
import org.eximeebpms.bpm.cockpit.plugin.resource.AbstractCockpitPluginRootResource;
import org.eximeebpms.bpm.cockpit.plugin.sample.SamplePlugin;
...
Note that a sub resource gets initialized by the plug-in when requests to `{engineName}/process-instance` are being made. That ensures that a Cockpit service is multi-tenancy ready out of the box (i.e. capable to work with all process engines provided by the EximeeBPMS).

A sub-resource may extend `org.eximeebpms.bpm.cockpit.plugin.resource.AbstractPluginResource` to get initialized with the correct process engine mappings. The resource shown below exposes our custom SQL query to the client when accessing the resource via `GET`.
...
import org.eximeebpms.bpm.cockpit.plugin.sample.db.ProcessInstanceCountDto;
...
It is possible to use the tenant check in the sub-resource of the cockpit plugin. Therefore, the sub-resource may extend `org.eximeebpms.bpm.cockpit.plugin.resource.AbstractPluginResource` to configure the custom query with the tenant check. See the following example for the tenant check usage.
...
>Consider reading about the [Structure of a Frontend Module](https://docs.eximeebpms.org/manual/latest/webapps/cockpit/extend/plugins/#structure-of-a-frontend-module) if you are interested in more details.
...
When deploying the extended EximeeBPMS webapplication, we can see the plug-in in action.
...
[1]: https://github.com/EximeeBPMS/eximeebpms/tree/main/webapps
