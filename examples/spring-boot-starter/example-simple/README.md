# Simple EximeeBPMS Spring Boot Application

This example only uses `eximeebpms-bpm-spring-boot-starter`, so it does not start an embedded tomcat and provides no REST API.
It demonstrates how EximeeBPMS can be used in combination with Spring Boot to spawn a node that:
...
1. To embed the EximeeBPMS engine you must add the following dependency to your `pom.xml`:
...
4. You can also put BPMN and DMN files in your classpath, they will be automatically deployed and registered within the process application.

5. When implementing the process application in [standard manner](https://docs.eximeebpms.org/manual/latest/user-guide/process-applications/the-process-application-class/),
...
```text
postDeploy: PostDeployEvent{processEngine=org.eximeebpms.bpm.engine.impl.ProcessEngineImpl@6973b51b}
...
preUndeploy: PreUndeployEvent{processEngine=org.eximeebpms.bpm.engine.impl.ProcessEngineImpl@3be4ea5e}
```