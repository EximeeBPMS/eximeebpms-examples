...
For additional information, please also see the [documentation](https://docs.eximeebpms.org/manual/latest/webapps/cockpit/extend/configuration/#bpmn-diagram-viewer-bpmn-js).
...
To include the bundled module in your Cockpit application, copy the [`custom-renderer-module-bundled.js` file](./dist/custom-renderer-module-bundled.js) into the `app/cockpit/scripts/` folder in your EximeeBPMS webapp distribution.
For the Tomcat distribution, this would be `server/apache-tomcat-X.X.XX/webapps/eximeebpms/app/cockpit/scripts/`.
