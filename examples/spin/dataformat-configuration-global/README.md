...
This example describes how to *globally* configure a data format provided by EximeeBPMS Spin.
...
In particular, we examine how to customize the serialization of the class [Car](src/main/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/Car.java):
...
`Car` references [Money](src/main/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/Money.java):
...
2. Read and run the [unit test](src/test/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/JacksonConfiguratorTest.java)
...
Configuring data formats requires to implement the SPI [DataFormatConfigurator](https://github.com/EximeeBPMS/eximeebpms-spin/blob/master/core/src/main/java/org/eximeebpms/spin/spi/DataFormatConfigurator.java) and declare implementations of it in a file `META-INF/services/org.eximeebpms.spin.spi.DataFormatConfigurator`. Here, the class [JacksonDataFormatConfigurator](src/main/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/JacksonDataFormatConfigurator.java) is such an implementation. It has the following contents:
...
The method `getDataFormatClass` declares the kind of data formats a configurator is able to configure. The method `configure` is the callback invoked by the Spin runtime after data format instantiation. Here, we register in form of the classes [MoneyJsonSerializer](src/main/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/MoneyJsonSerializer.java) and [MoneyJsonDeserializer](src/main/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/MoneyJsonDeserializer.java) a Jackson serializer and deserializer for the `Money` class. These make sure that a `Car` object is serialized in the way we need it.
...
In the [test case](src/test/java/org/eximeebpms/bpm/example/spin/dataformat/configuration/JacksonConfiguratorTest.java), we assert that the *price* property was successfully populated.

For this to work, it is crucial that the Spin runtime is able to lookup the `DataFormatConfigurator` implementation. That means, the artifact containing the data format configurator and the corresponding `META-INF/services` file must be on the classpath of the EximeeBPMS Spin process engine plugin.

You can find the Spin documentation on configuring data formats [here](https://docs.eximeebpms.org/manual/latest/reference/spin/extending-spin/#configuring-data-formats).
...
2. Make it available on the classpath of the artifact `eximeebpms-engine-plugin-spin`. Depending on the application server in use, this may be in varying places (see below).

Where to place the data format configuration JAR on which application server:

| Application Server           | Configuration                                                           |
| ---------------------------- | ----------------------------------------------------------------------- |
| WebLogic/Websphere           | Add the JAR and any dependencies that are not yet present to the EximeeBPMS EAR. Replace the deployed EximeeBPMS EAR by the updated one. |
| JBoss/Wildfly                | Deploy the JAR as a JBoss module. Create appropriate module dependencies for the JAR's dependencies. Keep in mind that all transitive dependencies must be declared as well. For this example, the dependencies should be `org.eximeebpms.spin.eximeebpms-spin-dataformat-json-jackson`, `org.eximeebpms.spin.eximeebpms-spin-core`, `com.fasterxml.jackson.core.jackson-databind` (with same version that `eximeebpms-spin-dataformat-json-jackson` uses). Edit `${JBOSS_HOME}/modules/org/eximeebpms/bpm/eximeebpms-engine-plugin-spin/main/module.xml`. Declare a dependency on the JAR module and make sure to set the attribute `services="import"`. |
