...
> EximeeBPMS Platform **enterprise edition**.

This quickstart demonstrates how to use the XSLT script engine extension to transform data in EximeeBPMS.
The example is *classless*, relying entirely on scripting and expression language.

> Note: on this JDK, the Groovy execution listener scripts fail with a `NoClassDefFoundError` /
> `ExceptionInInitializerError` inside `org.codehaus.groovy.vmplugin.v8.Java8#getValue` while Groovy
> compiles the script's AST — a known Groovy/JDK compatibility gap, reproduced identically with both
> Groovy 4.0.22 and the latest 5.0.8, unrelated to this rebrand.
...
<bpmn2:startEvent id="StartEvent_1" name="give input">
  <bpmn2:extensionElements>
    <camunda:executionListener event="end">
      <camunda:script scriptFormat="groovy" resource="org/eximeebpms/bpm/example/xsltexample/readXmlFile.groovy" />
...
```xml
<bpmn2:scriptTask id="ScriptTask_1" name="convert input" scriptFormat="xslt" camunda:resource="org/eximeebpms/bpm/example/xsltexample/example.xsl" camunda:resultVariable="xmlOutput">
  <bpmn2:extensionElements>
    <camunda:inputOutput>
...
<bpmn2:endEvent id="EndEvent_1" name="show output">
  <bpmn2:extensionElements>
    <camunda:executionListener event="start">
      <camunda:script scriptFormat="groovy" resource="org/eximeebpms/bpm/example/xsltexample/printResult.groovy" />
    </camunda:executionListener>
  </bpmn2:extensionElements>
</bpmn2:endEvent>
```
...
[3]: src/main/resources/org/eximeebpms/bpm/example/xsltexample/example.xsl
[4]: src/main/resources/org/eximeebpms/bpm/example/xsltexample/example.xml
[5]: src/test/resources/expected_result.xml
[6]: src/test/java/org/eximeebpms/bpm/example/xsltexample/XsltExampleTest.java
