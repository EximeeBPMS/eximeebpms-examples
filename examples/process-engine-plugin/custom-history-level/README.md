...
public void execute(DelegateExecution execution) throws Exception {
  ...
  execution.setVariable("eximeebpms-hist", "rocks");
}
```
...
The custom history level engine plugin can be activated in the `eximeebpms-per-process.cfg.xml`:

``` xml
<!-- activate bpmn parse listener as process engine plugin -->
<property name="processEnginePlugins">
  <list>
    <bean class="org.eximeebpms.bpm.example.CustomHistoryLevelProcessEnginePlugin" />
  </list>
</property>
```
...
To enable the custom history configure it also in the `eximeebpms-custom-variable.cfg.xml`:
...
[3]: src/main/java/org/eximeebpms/bpm/example/PerProcessHistoryLevel.java
[4]: src/test/java/org/eximeebpms/bpm/example/test/CustomVariableHistoryLevelTest.java
[5]: src/test/java/org/eximeebpms/bpm/example/test/PerProcessVariableHistoryLevelTest.java
