...
See the [user guide](https://docs.eximeebpms.org/manual/latest/user-guide/process-engine/multi-tenancy/) for a general introduction on multi-tenancy and the different options EximeeBPMS offers.


# Before Starting

Before starting, make sure to download the [EximeeBPMS WildFly distribution](https://repo1.maven.org/maven2/org/eximeebpms/bpm/wildfly/eximeebpms-bpm-wildfly/1.0.0/) and extract it to a folder. We will call this folder `$EXIMEEBPMS_HOME` in the following explanations.
...
* **JDBC URL**: jdbc:h2:./eximeebpms-h2-dbs/process-engine
* **User Name**: sa
* **Password**: sa
...
Execute the create scripts (`h2_engine_1.0.0.sql` and `h2_identity_1.0.0.sql`) after selecting the appropriate schema for the current connection:

```sql
set schema TENANT1;

<<paste sql/create/h2_engine_1.0.0.sql here>>
<<paste sql/create/h2_identity_1.0.0.sql here>>
...
Open the file `$WILDFLY_HOME/standalone/configuration/standalone.xml`. In that file, navigate to the configuration of the EximeeBPMS jboss subsystem, declared in an XML element `<subsystem xmlns="urn:org.eximeebpms.bpm.jboss:1.1">`. In this file, add two entries to the `<process-engines>` section (do *not* remove default engine configuration):
...
In the project, we have set up a plain EximeeBPMS EJB process application.

In [pom.xml](pom.xml), the `eximeebpms-engine-cdi-jakarta` and `eximeebpms-ejb-client-jakarta` dependencies are added:
...
To showcase the programming model for multi-tenancy with CDI, we have added a simple REST resource that returns all deployed process definitions for a process engine. The resource has the following [source code](src/main/java/org/eximeebpms/bpm/tutorial/multitenancy/ProcessDefinitionResource.java):
...
We want the injected process engine to always be the one that matches the current tenant making a REST request. For this matter, we have added a request-scoped [tenant bean](src/main/java/org/eximeebpms/bpm/tutorial/multitenancy/Tenant.java):
...
To populate this bean with the tenant ID for the current user, we add a [RestEasy interceptor](src/main/java/org/eximeebpms/bpm/tutorial/multitenancy/TenantInterceptor.java). This interceptor is called before a REST request is dispatched to the `ProcessDefinitionResource`. It has the following content:
...
To resolve the process engine based on the tenant, we have [specialized the process engine producer](src/main/java/org/eximeebpms/bpm/tutorial/multitenancy/TenantAwareProcessEngineServicesProducer.java) bean as follows:
...
Go to EximeeBPMS Cockpit and switch the engine to `tenant1` on the following URL (you will be asked to create an admin user first):
http://localhost:8080/eximeebpms/app/cockpit/tenant1/
...
The test class [ProcessIntegrationTest](src/test/java/org/eximeebpms/bpm/tutorial/multitenancy/ProcessIntegrationTest.java) uses Arquillian to verify the behavior. 

Follow the steps to run the test:

* Download the [EximeeBPMS WildFly distribution](https://repo1.maven.org/maven2/org/eximeebpms/bpm/wildfly/eximeebpms-bpm-wildfly/1.0.0/)
* Replace the `server/wildfly-{version}/standalone/configuration/standalone.xml` with
