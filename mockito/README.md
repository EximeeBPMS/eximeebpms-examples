...
eximeebpms-bpm-mockito is a Mockito extension for the EximeeBPMS process engine that aims to simplify and
automate mocking of process applications. It is a fork of `camunda-bpm-mockito`
(now `camunda-community-hub/camunda-platform-7-mockito`), rebranded and adapted for the
renamed engine - see [Maintainer](#maintainer) for original-author attribution.
...
**Note:** not yet published to Maven Central - the coordinates below won't resolve
remotely yet. Clone this repository and run `mvn install` in `mockito/` first.
Publishing is mechanically ready (see `.github/workflows/release-mockito.yml`) but no
release has been run yet; the first release will be `1.0.0` (a genuine fork of a
long-mature, stable library, unlike `assert-scenario/`'s alpha status).

Just include eximeebpms-bpm-mockito in the test scope of your project:

```xml
<dependency>
  <groupId>org.eximeebpms.bpm.extension.mockito</groupId>
  <artifactId>eximeebpms-bpm-mockito</artifactId>
  <scope>test</scope>
...
```
   testImplementation("org.eximeebpms.bpm.extension.mockito:eximeebpms-bpm-mockito:1.0.0")
```
...
With the QueryMocks extension, you can do all this in just one line of code, see [QueryMocksExample.java](src/test/java/org/eximeebpms/bpm/extension/mockito/QueryMocksExample.java).
...
More examples could be found in [CallActivityMockExampleTest.java](src/test/java/org/eximeebpms/bpm/extension/mockito/CallActivityMockExampleTest.java).
...
with your process engine. EximeeBPMS provides a fluent builder API for creation a message correlation
and running it.
...
Not applicable yet for this fork - no release has been made. See the original
project's release notes for `camunda-bpm-mockito`'s history up to the `5.15.0`
tag this fork is based on: https://camunda.github.io/camunda-bpm-mockito/release-notes/
...
* [Issue Tracker](https://github.com/EximeeBPMS/eximeebpms-examples/issues)
* [Contributing](CONTRIBUTING.md)

## Maintainer

Forked and maintained by the EximeeBPMS Community. Original authors of
`camunda-bpm-mockito`, whose work this fork is based on:
