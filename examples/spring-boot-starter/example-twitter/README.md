...
It uses `eximeebpms-bpm-spring-boot-starter-webapp` and thus embed Tomcat as a web container.

The example contains:
- a process application with one process deployed on the EximeeBPMS engine
...
1. To embed the EximeeBPMS engine you must add following dependency to your `pom.xml`:
...
3. You can also put BPMN and DMN files in your classpath, they will be automatically deployed and registered within the process application.

4. You can configure your Spring Boot application using `application.yaml` file. All possible EximeeBPMS-specific configuration parameters are listed [here](https://docs.eximeebpms.org/manual/latest/user-guide/spring-boot-integration/configuration/)
