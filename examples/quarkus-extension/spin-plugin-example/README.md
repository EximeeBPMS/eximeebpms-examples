# EximeeBPMS Runtime: Quarkus Engine Extension Spin Example

This example uses Quarkus, the Supersonic Subatomic Java Framework, 
in combination with the extension `eximeebpms-bpm-quarkus-engine`.

If you want to learn more about Quarkus, please visit: https://quarkus.io/.

The example demonstrates how you can use the EximeeBPMS Platform Runtime Engine in combination with Quarkus to spawn a node that:

* Connects to an H2 on-disk database.
* [Registers the EximeeBPMS Spin Process Engine Plugin](org/eximeebpms/bpm/quarkus/example/EngineConfiguration.java).
* Bootstraps a process engine.
* [Automatically deploys](org/eximeebpms/bpm/quarkus/example/ResourceDeployment.java) the resource [process.bpmn](src/main/resources/process.bpmn) to the process engine.
* Exposes a REST endpoint that:
  * starts a process instance
  * stores the JSON payload as a process variable
  * calls the [StoreOderItemService](org/eximeebpms/bpm/quarkus/example/service/StoreOrderItemService.java) bean.
...
The application is now runnable using `java -jar target/eximeebpms-bpm-quarkus-example-spin-plugin-1.0.0-SNAPSHOT-runner.jar`.
